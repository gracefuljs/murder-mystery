...
function distributeData(suspects, data){
	...
	suspectList.forEach((suspect, index) => {
		validData = dataList.filter( (s) => isSelfFilter(suspect, s) );
...
function generateAllStatements(suspectList, locationObjects, crimeData){
	...
	suspectList.forEach( (suspect, index) => {
		...
		// console.group(`${suspect.name} says:`);
		// console.log(suspect.statements.alibi);
		// console.log(suspect.statements.motive);
		// console.log(suspect.statements.weapon);
		// console.groupEnd();
	});
